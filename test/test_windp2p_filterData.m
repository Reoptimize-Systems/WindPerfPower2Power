...
% % % Potential for "for" loop here
n=2; % n can be used as a for loop, it selects the "pair" within data_info.All_combinations that is being considered.


% % % % % % [data,base_kW,ControlWT_S_No,TestWT_S_No] = FCN_load_data(SELECT_DATA);
[data,base_kW,ControlWT_S_No,TestWT_S_No] = FCN_load_data(SELECT_DATA,'n',n,'data_info',data_info);
...
if exist('data.ControlWTPre.AirPressure','var')
...
for idx=1:length(Columns_of_interest) % Legend for dots

    Legend{idx} = [ num2str(nacelle_pos_edges(Columns_of_interest(idx))) ' - ' num2str(nacelle_pos_edges(Columns_of_interest(idx)+1))];

end

for idx=1:length(Columns_of_interest) % Legend for lines

    Legend{idx+length(Columns_of_interest)} = [ num2str(nacelle_pos_edges(Columns_of_interest(idx))) ' - ' num2str(nacelle_pos_edges(Columns_of_interest(idx)+1))];
